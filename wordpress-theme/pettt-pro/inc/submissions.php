...
add_shortcode('ninjapet_submit_service', function(){
    if (!empty($_POST['ninjapet_submit_service']) && isset($_POST['ninjapet_service_nonce']) && wp_verify_nonce($_POST['ninjapet_service_nonce'], 'ninjapet_submit_service')) {
        ...
        return '<div class="np-error">ثبت اطلاعات انجام نشد. لطفاً دوباره تلاش کنید.</div>';
    }
    ob_start(); ?>
    <form class="np-submit-form" method="post">
      ...
      <label>نام مسئول<input name="manager_name"></label><label>شماره تماس<input name="phone" required></label><label>واتساپ<input name="whatsapp"></label><label>اینستاگرام<input name="instagram" placeholder="@ninjapet"></label>
      <label>استان<?php echo ninjapet_province_select('province'); ?></label><label>شهر<?php echo ninjapet_city_select('city'); ?></label>
...
add_shortcode('ninjapet_submit_explore', function(){
    ...
    if (!empty($_POST['ninjapet_submit_explore']) && isset($_POST['ninjapet_explore_nonce']) && wp_verify_nonce($_POST['ninjapet_explore_nonce'], 'ninjapet_submit_explore')) {
        ...
        if($id && !is_wp_error($id)){ update_post_meta($id,'_pettt_pet_name',sanitize_text_field($_POST['pet_name'] ?? '')); update_post_meta($id,'_pettt_pet_breed',sanitize_text_field($_POST['pet_breed'] ?? '')); update_post_meta($id,'_pettt_pet_food',sanitize_text_field($_POST['pet_food'] ?? '')); $msg='<div class="np-success">پست شما ذخیره شد و در انتظار تأیید مدیر است.</div>'; }
        else $msg='<div class="np-error">ثبت پست انجام نشد.</div>';
    }
