...
add_action('save_post_pettt_service', function ($post_id) {
    ...
    $keys = ['_pettt_instagram','_pettt_website','_pettt_working_hours','_pettt_video_url','_pettt_video_embed','_pettt_google_map_embed'];
    foreach ($keys as $key) {
...
function pettt_service_instagram_url($value) {
    ...
    $handle = ltrim($value, '@');
    return esc_url('https://instagram.com/' . $handle);
}
